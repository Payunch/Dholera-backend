 ...
 */
// onboardRateLimiter removed

exports.onboardLead = async (req, res) => {
  try {
...
exports.saveDirect = async (req, res) => {
  req.url = '/onboard';
  return exports.onboardLead(req, res);
};

// POST track returning verified user
exports.trackReturning = async (req, res) => {
  try {
...
exports.verifyLeadToken = async (req, res) => {
  try {
    const leadToken = extractToken(req.headers['authorization']);
...
exports.getWhatsappUrl = async (req, res) => {
  try {
    const lead = await Lead.findByPk(req.params.id);
    if (!lead) return res.status(404).json({ error: 'Lead not found' });
...
exports.systemRestore = async (req, res) => {
  try {
...
exports.deleteLead = async (req, res) => {
  try {
    ...
    await logAuditEvent({
      ...
      ip: req.ip,
      userAgent: req.headers['user-agent'],
      details: { leadId: req.params.id, name: lead.name, phone: lead.phone }
    });
