...
| **02-Jul-2026 12:00** | Authentication | Firebase SMS OTP failing due to Google Cloud / Blaze Plan billing issues, completely blocking user verification and automated behavioral follow-ups. | **WhatsApp OTP Migration:** Completely bypassed Firebase Auth for SMS. Engineered a secure, custom OTP generation and verification system (`OtpVerification` model) using the **Meta WhatsApp API**. OTPs are now sent instantly via WhatsApp (`otp_verification` template), saving costs and ensuring 100% delivery. |
